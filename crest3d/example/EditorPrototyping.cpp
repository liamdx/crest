...
EditorPrototyping::EditorPrototyping(EngineManager* em)
{
	...
	cyborgEntity = engineManager->scene->AddModelEntity(m->asset);
...
*/
	levelEntity = engineManager->scene->AddModelEntity(level->asset);
	...
	animEntity = engineManager->scene->AddAnimatedModelEntity(animatedModel->asset);
	animEntity->transform->addPosition(glm::vec3(-4.0, 0.0, 0.0));

	dirLight = engineManager->scene->AddDirectionalLightEntity();
...
void EditorPrototyping::initBehaviour()
{
	...
	auto pl1 = engineManager->scene->AddPointLightEntity();
	...
	auto pl2 = engineManager->scene->AddPointLightEntity();
	...
	auto pl3 = engineManager->scene->AddPointLightEntity();
	...
	auto pl4 = engineManager->scene->AddPointLightEntity();
	...
	auto pl5 = engineManager->scene->AddPointLightEntity();
	...
	auto pl6 = engineManager->scene->AddPointLightEntity();
	...
	cameraEntity = engineManager->scene->AddCameraEntity();
	cameraEntity->AddComponent(new CameraControllerComponent(cameraEntity, engineManager->input));
...
void EditorPrototyping::uiBehaviour(float deltaTime)
{
	if (ImGui::Begin("Editor Prototyping", NULL, ImGuiWindowFlags_NoMove));
	{
		...
		{
			engineManager->scene->AddPointLightEntity();
		}
		...
		// load model
		{
			...
			if (ImGui::Button("Load New Model Entity"))
			{
				...
				engineManager->scene->AddModelEntity(_model->asset);
			}
		}
		// load animated model
		{
			...
			if (ImGui::Button("Load New AnimatedModel Entity"))
			{
				...
				engineManager->scene->AddAnimatedModelEntity(_anim->asset);
			}
		}
